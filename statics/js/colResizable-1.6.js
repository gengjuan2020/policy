...
(function($){ 	
	...
	var memento = function(t, th){ 
		...
		if(th){										//in deserialization mode (after a postback)
			...
			for(i=0;i<t.ln;i++)
				t.cg.eq(i).css("width", aux[i]);	//this cmd is required in order to create an inline CSS rule with higher precedence than an existing CSS class in the "col" elements
		...
		for(var t in tables){
            if( tables.hasOwnProperty( t ) ) {
                ...
                if (t.f) {                  //in fixed mode
                    ...
                    //each browser. In the beginning i had a big switch for each browser, but since the cmd
